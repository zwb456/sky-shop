...
const { sendMessage, RndNum } = require('../../utils/message2.js')
const {
  register,
  login,
  changePassword,
  ...
  sendMessage(phoneNum, VerificationCode).then((result) => {
    ...
    if (result.message) {
      ...
      res.json({
        ...
        message: message.split('Permits')[0],
      })
      // return
    } else {
      res.json({
...
router.post('/register', (req, res) => {
  ...
  console.log(username, password, email, VerificationCode,temp)
  ...
  register(sql, [Date.now(), username, password, email, Date.now()])
    .then((data) => {
      res.json(data)
    })
    .catch((err) => {
...
router.post('/changePassword', (req, res) => {
  ...
  changePassword(sql, [password, username])
    .then((data) => {
      res.json(data)
    })
    .catch((err) => {
...
router.post('/updateUserInfo', (req, res) => {
  ...
  let sql =
    'update t_customer set avatar=?,nickname=?,create_time=?,username=?,email=?,tel=? where id=?'
  updateUserInfo(sql, [avatar, nickname, Date.now(), username, email, tel, id])
    .then((data) => {
      res.json(data)
    })
    .catch((err) => {
